...
struct RobloxProcess
{
	...
	bool Attach(HANDLE process, int retry_count)
	{
		...
		{
			NotifyError("Koronefpsunlocker Error", "Failed to get process base! Restart Korone FPS Unlocker or, if you are on a 64-bit operating system, make sure you are using the 64-bit version of Korone FPS Unlocker.");
			retries_left = -1;
			return false;
		}
		else
